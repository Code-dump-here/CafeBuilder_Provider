  ...
  return (
    <div className="flex flex-col gap-1.5 rounded-md border border-border/60 bg-background/40">
      <button
        ...
        onClick={() => setOpen((v) => !v)}
        aria-expanded={open}
        className="flex items-center justify-between gap-2 px-3 py-2 text-left text-2xs font-semibold uppercase tracking-wide text-muted-foreground hover:text-foreground"
      >
        <span>{label}</span>
        <span className="text-xs">{open ? "Hide" : "Show"}</span>
      </button>
      {open ? (
        <pre className="max-h-72 overflow-auto border-t border-border/60 px-3 py-2 font-mono text-xs leading-relaxed text-foreground/90 wrap-break-word">
          {children}
        </pre>
      ) : null}
    </div>
        ...
        {zones.map((zone, idx) => {
          ...
          return (
            <div
              ...
              className={cn(
                "absolute flex items-center justify-center rounded-sm border border-border/70 p-1 text-xs font-medium text-foreground/90 backdrop-blur-sm",
                palette,
              )}
              style={{
    ...
    <ol className="flex flex-col gap-2 lg:flex-row lg:items-stretch">
      {steps.map((step, idx) => (
        ...
        >
          <span
            aria-hidden
            className="inline-flex size-6 shrink-0 items-center justify-center rounded-full bg-primary/15 text-xs font-semibold text-primary"
          >
            {idx + 1}
          </span>
          <div className="min-w-0 flex-1">
        ...
        >
          <header className="flex items-start gap-2">
            ...
            <div className="min-w-0 flex-1">
              ...
              {risk.mitigation ? (
                <p className="mt-1 text-2xs font-semibold uppercase tracking-wide opacity-80">
