...
| ------------------------- | ---------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------- |
| Account `role`            | `owner` \| `provider` \| `admin`                                                   | Actor type. Designer/constructor is NOT a role — it's a provider capability                    |
| Provider `capability`     | `designer` \| `constructor` \| `both`                                              | What the provider can do; must match post `serviceKind` / engagement `contractType`            |
| Provider `providerType`   | `individual` \| `company`                                                          | Provider profile kind                                                                          |
| Post `serviceKind`        | `design` \| `construction` \| `both`                                               | What the owner is hiring for                                                                   |
| Post `status`             | `open` \| `closed` \| `cancelled`                                                  | Post lifecycle; `open` filter auto-hides expired posts                                         |
| Apply `status`            | `pending` \| `accepted` \| `rejected`                                              | Application lifecycle                                                                          |
| Engagement `status`       | `requested` → `accepted` \| `rejected`; `accepted` → `completed` \| `terminated`   | RELATIONSHIP state, not progress. "In progress" = derived: `accepted` + a `confirmed` contract |
| Engagement `contractType` | `design` \| `construction` \| `both`                                               | Scope of the engagement                                                                        |
...
| Design `type`             | `concept` \| `layout_2d` \| `render_3d` \| `technical_drawing`                     | Kind of design deliverable                                                                     |
| Item/Task `status`        | `pending` → `in_progress` → `completed`                                            | Construction milestone & task progress (sequential only)                                       |
| AI `state`                | `queued` \| `processing` \| `completed` \| `failed`                                | AI job status — FE polls until completed/failed                                                |
...
### Accounts — `api/accounts` (admin)

CRUD: `GET` (paged) / `GET /{id}` / `POST { email*, password* (≥8), phone, role*, status }` / `PUT /{id} { phone, role, status }` / `DELETE /{id}`.
`AccountResponse`: `{ id, email, phone, role, status, emailVerifiedAt, createdAt, updatedAt }`

### Shop Owners — `api/shop-owners` (owner profile, create after register)
