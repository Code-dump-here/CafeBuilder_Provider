 ...
 * to mock data).
 */
let CHAT_API_OVERRIDE: null | ((projectWorkingId: string) => MessageThread[] | null) = null;

/** Override that bypasses the API entirely — used in dev / demo mode. */
export const __setChatApiOverride = (
  next: (projectWorkingId: string) => MessageThread[] | null,
) => {
...
export async function fetchMessageThreadsFromApi(
  ...
  try {
    ...
    return result.items.map((conv) =>
      // The conversations endpoint does not return the project id; the
      // caller supplies it. Empty string is the "unknown id" placeholder now
      // that ids are uuids.
      apiConversationToThread(conv, ""),
    );
  } catch (err) {
