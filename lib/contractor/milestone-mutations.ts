...
import type { MilestoneTask, TaskMetaMap } from "@/lib/contractor/milestone-mgmt-state";
import { taskKey } from "@/lib/contractor/milestone-mgmt-state";
  ...
  switch (action.type) {
    ...
    case "set_phase_status": {
      return mutatePhase(state, action.phaseId, (p) => {
        ...
        // Status transitions cascade to progress for the "terminal" cases
        // — completed = 100%, upcoming = whatever the user has set (could
        // be 0 but they may want a rough estimate).
    ...
    case "remove_phase": {
      ...
      // Strip completion + meta state for the deleted phase.
    ...
    case "set_task_meta": {
      ...
      // Drop the entry if everything is empty — sparse is the design.
      const isEmpty =
    ...
    case "remove_task": {
      ...
      const nextTasks = phase
        ...
        : undefined;
      // The removed task's meta entry is by index; everything after
      // shifts down by one so the keys still line up.
...
/**
 * Derive the short label shown on the track node. We try a few cuts so
 * labels like "Studs, drywall, MEP rough-in" collapse to "Framing"
 * (which is the existing shorthand) — fallback to first word / first
 * 10 chars.
 */
...
/** Convenience: count of completed tasks across a phase. */
...
/**
 * Recompute phase progress from per-task completion when a task flips
 * done. We don't apply this automatically inside the reducer (callers
 * want explicit control) — the page applies it after a `toggle_task`
...
/**
 * Re-key task meta entries after a task is removed: drop the deleted
 * key and decrement every subsequent index by one.
 */
