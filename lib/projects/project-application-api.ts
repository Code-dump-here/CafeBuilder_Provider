 ...
 *
 * Error shape:
 *   - 400  — invalid body (negative duration, missing proposal, etc.).
 *   - 401  — no / expired access token. The interceptor will try a
 *            refresh once before bubbling the error.
 *   - 403  — the caller is not a provider (role mismatch) or already
 *            applied to the same post.
 *   - 404  — postId doesn't exist or isn't in `open` status.
 *   - 409  — duplicate application (idempotency key on the server).
 */
